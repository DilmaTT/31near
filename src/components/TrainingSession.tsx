...
import { allPossibleHands, generateBorderPlusOneHands, getCombinations, TOTAL_POKER_COMBINATIONS } from "@/lib/poker-utils";
...
export const TrainingSession = ({ training, onStop }: TrainingSessionProps) => {
  ...
  useEffect(() => {
    ...
    if (training.type === 'classic') {
      if (training.subtype === 'all-hands') {
        ...
        const handPool = generateBorderPlusOneHands(currentRange.hands);
  ...
  }, [training.type, training.subtype, currentRange]);
